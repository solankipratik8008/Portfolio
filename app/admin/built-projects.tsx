...
function ImageUploadField({
  ...
  return (
    <View style={imgStyles.container}>
      ...
      {value ? (
        <Image source={{ uri: value }} style={imgStyles.preview} resizeMode="cover" />
      ) : null}
      {Platform.OS === 'web' && (
        <input
          ref={inputRef}
      ...
      <Pressable onPress={() => inputRef.current?.click()} style={imgStyles.uploadBtn}>
        {uploading ? (
          <ActivityIndicator size="small" color={COLORS.accentPrimary} />
        ) : (
          <Text style={imgStyles.uploadBtnText}>{value ? 'Change Image' : '+ Upload Image'}</Text>
        )}
      </Pressable>
      <AdminFormField
      ...
      renderForm={(formData, setFormData) => (
        <>
          ...
          <ImageUploadField
            ...
            storagePath={`builtProjects/${formData.title || 'project'}_${Date.now()}`}
          />
          <AdminFormField
          ...
          />
          <AdminFormField
            label="Icon (Ionicon name)"
            value={formData.icon || ''}
            onChangeText={(v) => setFormData((p: any) => ({ ...p, icon: v }))}
            placeholder="e.g. cloud, airplane, cafe"
          />
          <AdminFormField
            label="Order"
